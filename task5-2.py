...
def k_means_clustering(T, k, seed):
    ...
    for numIteration in range(tmax): #for every iteration of Loyd's algorithm
        ...
        for i in range(len(clusters)): #for every cluster (here len(clusters)=k)
            newCenterTrajectory = approach2(clusters[i]) #calculate the center trajectory for each cluster or group of trajectories
        ...
        if all(dtw(new_centers[i],centers[i])<threshold for i in range(k)): #here we assume the index ordering was maintained
            break
        centers = new_centers 
